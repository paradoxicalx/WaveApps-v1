<?php
if (isset($_GET['e']) && $_GET['e'] === "router") {
  $report_array = [];
  $status = true;
  // cek apakah input diijikan dalam mode multiple edit.
  if (count($_POST['id']) > 1) {
    $cek_mustempty = ["nasname", "shortname"];
  ...
  $cek_empty_field = ["shortname", "nasname", "type", "ports", "secret", "server", "community", "description"];
  ...
  if ($status = true) {
    ...
    for ($i = 0; $i < count($id); $i++) {
      ...
      $update = SqlQuUpdate("radius.nas", $_POST, "id", "$id[$i]");
      if ($update['status'] == "success") {
        InputLog($_SESSION['name'],"radius","Router changed [".$nasname[0]['shortname']."]");
        $count++;
      }
    }
    if ($count === count($id)) {
