...
if (isset($_GET['e']) && $_GET['e'] === "group") {
  ...
  $cek_empty_field = ["groupname", "profile", "dllimit", "uplimit", "rate", "burst", "threshold", "bursttime", "maxsess"];
  ...
  if ($status = true) {
    ...
    for ($i = 0; $i < count($id); $i++) {
      ...
      // Update Max Session.
      if (isset($_POST['maxsess'])) {
        $changeid = SqlQuAssoc("SELECT id FROM radius.radgroupcheck WHERE `groupname` = '$oldname' AND `attribute` = 'Simultaneous-Use'");
        $changeid = $changeid[0]['id'];
        if (empty($changeid) or $changeid == "") {
          $update = SqlQuInsert("radius.radgroupcheck",
                    [
                      "groupname" => $oldname,
          ...
          if ($update['status'] !== "success") { $error = true; $log[] = "change max. session failed";}
        } else {
          $update = SqlQuUpdate("radius.radgroupcheck",
                   [
                     "value" => $_POST['maxsess']
                   ],
                   "id", $changeid);
      ...
      if ($error === false) {
        InputLog($_SESSION['name'],"radius","Group changed [".$oldname."]");
        $count++;
      }
    }
    if ($count === count($id)) {
