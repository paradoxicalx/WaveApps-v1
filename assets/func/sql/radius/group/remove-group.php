...
if (isset($_GET['r'])) {
  if ($_GET['r'] === "group") {
    ...
    for ($i = 0; $i < count($id); $i++) {
      ...
      if ($error === false) {
        InputLog($_SESSION['name'],"radius","Group removed [".$groupname."]");
        $count++;
      }
    }
    if ($count === count($id)) {
  ...
  if ($_GET['r'] === "group-data") {
    ...
    if ($data == "Download Limit") {
      for ($i = 0; $i < count($id); $i++) {
        ...
        if ($remove['status'] == "success") {
          InputLog($_SESSION['name'],"radius","Download Limit data removed [".$groupname."]");
          $count++;
        }
      }
    }
    if ($data == "Upload Limit") {
      for ($i = 0; $i < count($id); $i++) {
        ...
        $remove = SqlQuRemove("radius.radgroupreply", "id", $radreply);
        if ($remove['status'] == "success") {
          InputLog($_SESSION['name'],"radius","Upload Limit data removed [".$groupname."]");
          $count++;
        }
      }
    }
    if ($data == "Max. Session") {
      for ($i = 0; $i < count($id); $i++) {
        $groupname = sqlQuAssoc("SELECT groupname FROM radius.radusergroup WHERE `id` = '$id[$i]'");
        $groupname = $groupname[0]['groupname'];
        $radreply = SqlQuAssoc("SELECT id FROM radius.radgroupcheck WHERE `groupname` = '$groupname' AND `attribute` = 'Simultaneous-Use'");
        $radreply = $radreply[0]['id'];
        $remove = SqlQuRemove("radius.radgroupcheck", "id", $radreply);
        if ($remove['status'] == "success") {
          InputLog($_SESSION['name'],"radius","Max. session data removed [".$groupname."]");
          $count++;
        }
      }
    }
    if ($data == "Rate Limit") {
      for ($i = 0; $i < count($id); $i++) {
        ...
        $remove = SqlQuRemove("radius.radgroupreply", "id", $radreply);
        if ($remove['status'] == "success") {
          InputLog($_SESSION['name'],"radius","Rate Limit data removed [".$groupname."]");
          $count++;
        }
      }
    }
    if ($data == "Profile") {
      for ($i = 0; $i < count($id); $i++) {
        ...
        $remove = SqlQuRemove("radius.radgroupreply", "id", $radreply);
        if ($remove['status'] == "success") {
          InputLog($_SESSION['name'],"radius","Profile data removed [".$groupname."]");
          $count++;
        }
      }
    }
