...
if (isset($_GET['e']) && $_GET['e'] === "user") {
  ...
  if ($status = true) {
    ...
    for ($i = 0; $i < count($id); $i++) {
      ...
      if ($error === false) {
        InputLog($_SESSION['name'],"radius","User changed [".$oldname."]");
        $count++;
      }
    }
    if ($count === count($id)) {
...
if (isset($_GET['status'])) {
  ...
  if ($_POST['status'] === "enable") {
    ...
    unset($_POST['id']);
    for ($i = 0; $i < count($id); $i++) {
      $detail = SqlQuAssoc("SELECT * FROM radius.radusergroup WHERE `username` = '".$id[$i]."'");
      $username = $detail[0]['username'];
      //cek member dihapus atau tidak
      $memberdata = SqlQuAssoc("SELECT * FROM wavenet.tb_user WHERE `id` = '".$detail[0]['guid']."'");
      if ($memberdata[0]['deleted'] > 0) {
        $report = ["status" => "failed"];
        $report[] = ["error" => "Member not existing" ];
        echo json_encode($report);
        exit;
      }
      $update = SqlQuRDelete("radius.radcheck", "`username`='$username' AND `attribute`='Auth-Type' AND `value`='Reject'");
      $update = SqlQuUpdate("radius.radusergroup",
               [
                 "status" => "enable"
               ],
               "username", $username);
      echo json_encode($update);
      InputLog($_SESSION['name'],"radius","User $username enable");
    }
  }
  ...
  if ($_POST['status'] === "disable") {
    ...
    for ($i = 0; $i < count($id); $i++) {
      $detail = SqlQuAssoc("SELECT * FROM radius.radusergroup WHERE `username` = '".$id[$i]."'");
      $username = $detail[0]['username'];
      $update = SqlQuInsert("radius.radcheck",
                [
                  "username" => $username,
                  "attribute" => "Auth-Type",
                  "op" => ":=",
      ...
      InputLog($_SESSION['name'],"radius","User $username disable");
      echo json_encode($update);

      $naslist = SqlQuAssoc("SELECT * FROM radius.nas");
      foreach ($naslist as $key) {
        $session = SqlQuAssoc("SELECT * FROM radius.radacct WHERE `acctstoptime` IS NULL AND `username` = '$username'");
        foreach ($session as $key1) {
          exec("echo 'User-Name=$username' | radclient -x ".$key1['nasipaddress'].":3799 disconnect '".$key['secret']."'");
        }
      }
    }
  }
