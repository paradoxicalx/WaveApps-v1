...
if (isset($_GET['r'])) {
  if ($_GET['r'] === "user") {
    ...
    for ($i = 0; $i < count($id); $i++) {
      ...
      if ($error === false) {
        InputLog($_SESSION['name'],"radius","User removed [".$username."]");
        $count++;
      }
    }
    if ($count === count($id)) {
  ...
  if ($_GET['r'] === "user-data") {
    ...
    if ($data == "Ipv4") {
      for ($i = 0; $i < count($id); $i++) {
        ...
        if ($update['status'] == "success") {
          InputLog($_SESSION['name'],"radius","IPv4 data removed [".$username."]");
          $count++;
        }
      }
    }
    if ($data == "Download Limit") {
      for ($i = 0; $i < count($id); $i++) {
        ...
        $remove = SqlQuRemove("radius.radreply", "id", $radreply);
        if ($remove['status'] == "success") {
          InputLog($_SESSION['name'],"radius","Download limit data removed [".$username."]");
          $count++;
        }
      }
    }
    if ($data == "Upload Limit") {
      for ($i = 0; $i < count($id); $i++) {
        ...
        $remove = SqlQuRemove("radius.radreply", "id", $radreply);
        if ($remove['status'] == "success") {
          InputLog($_SESSION['name'],"radius","Upload limit data removed [".$username."]");
          $count++;
        }
      }
    }
    if ($data == "Rate Limit") {
      for ($i = 0; $i < count($id); $i++) {
        ...
        $remove = SqlQuRemove("radius.radreply", "id", $radreply);
        if ($remove['status'] == "success") {
          InputLog($_SESSION['name'],"radius","Rate limit data removed [".$username."]");
          $count++;
        }
      }
    }
