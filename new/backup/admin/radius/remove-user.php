          ...
          <div class="box-body">
            <div class="pretty p-default p-round p-thick  margin">
              ...
              <div class="state p-primary-o">
                <label>I understand. Continue to remove radius user!</label>
              </div>
            </div>
            <button id="remove-user" class="btn btn-block btn-danger btn-lg" disabled>
              <i class="fas fa-ban"></i> Remove Radius User
            </button>
          </div>
        </div>
      </div>
...
$('input#agree').on('click', function() {
  if ($("#agree").is(':checked')) {
    $("#remove-user").attr("disabled", false);
  } else {
    $("#remove-user").attr("disabled", true);
  }
});
...
$('#remove-user').on('click', function() {
  ...
  $.post("radius/sql-proc.php?r=user", {
    id: memberid
  },
  function(data) {
    ...
    if (status != "success") {
      ...
      $("#info").load( "../include/alert.php #callout-danger", function() {
        $("#callout-title-danger").html("Cannot Remove User!");
      });
    } else {
      ...
      $("#info").load( "../include/alert.php #callout-success", function() {
        $('#callout-title-success').html("Success Remove "+json[0]['count']+" Radius User!");
      });
    }
  });
})
...
$('#remove-data').on('click', function() {
  ...
  $.post("radius/sql-proc.php?r=user-data", {
    id: memberid,
