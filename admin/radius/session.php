    ...
    <div class="container-fluid">
      ...
      <ul class="nav navbar-nav nav-menu">
        <li class="dropdown">
          ...
          <ul class="dropdown-menu" role="menu">
            <li><a class="selected-dc pointer">
              <i class="fas fa-skull-crossbones text-red icoinput"></i> Terminate
            </a></li>
      ...
      <div class="collapse navbar-collapse" id="navbar-collapse">
        <ul class="nav navbar-nav">
          <li url="radius/sql-proc.php?qs=active" class="menu-nav info active"><a href="#">Active</a></li>
          <li url="radius/sql-proc.php?qs=today"class="menu-nav warning" ><a href="#">Today</a></li>
          <li url="radius/sql-proc.php?qs=all" class="menu-nav danger"><a href="#">All Session</a></li>
        </ul>
      </div>
    </div>
  </nav>
...
Table1Gen("radius/sql-proc.php?qs=active",
function ( row, data, index ) {
  $('td', row).eq(5).html(data[5]+"<i class='text-red pull-right fas fa-arrow-up'></i>");
  $('td', row).eq(6).html(data[6]+"<i class='text-blue pull-right fas fa-arrow-down'></i>");
});
...
$('.selected-dc').on('click', function() {
  $('#modal-default').modal('show');
  $(".modal-dialog").removeClass("modal-lg");
  $('#modal-title-default').text("Terminate session");
  $('#modal-body-default').load("radius/terminate.php");
});
</script>
